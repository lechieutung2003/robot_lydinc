    ...
    for root, dirs, files in os.walk(image_path):
        for filename in files:
            if filename.lower().endswith(('.png', '.jpg', '.jpeg')):
                full_path = os.path.join(root, filename)  # Sử dụng root để lấy đường dẫn đầy đủ
                try:
                ...
                detection_results = detector.detect(frame)  # Giả sử detect trả về danh sách các bounding box [(x, y, w, h), ...]
                ...
                if detection_results:
                    ...
                    for index, face_data in enumerate(detected_faces):
                        ...
                        face_image = frame[y:h, x:w]  # Sửa đoạn này để cắt đúng khuôn mặt
                        
                        if face_image.size == 0:  # Kiểm tra nếu face_image rỗng hoặc kích thước không hợp lệ
                            print(f"Error with image: {full_path}")  # In ra đường dẫn ảnh lỗi
                            continue
                        
                        face_image = cv2.resize(face_image, (256, 256))  # Resize ảnh về 256x256
                        
                        timestamp = datetime.now().strftime('%Y%m%d%H%M%S')
                        save_path = os.path.join(image_save_path, f"face_{timestamp}_{index}.jpg")
...
def save_detected_faces_video(directory_path, image_save_path, detector):
    ...
    for root, dirs, files in os.walk(directory_path):
        ...
        for filename in files:
            ...
            if filename.lower().endswith(('.mp4', '.avi', '.mov', '.mkv', 'MOV', 'MP4', 'AVI', 'MKV')):
                    ...
                    if frame_index % frame_interval == 0:
                        ...
                        if detection_results:
                            ...
                            for index, face_data in enumerate(detected_faces):
                                ...
                                timestamp = datetime.now().strftime('%Y%m%d%H%M%S')
                                save_path = os.path.join(image_save_path, f"face_{timestamp}_{frame_index}_{index}.jpg")
                                cv2.imwrite(save_path, face_image)
...
if __name__ == "__main__":
    image_path = "F:\LYDINC\Data origin\Robot"
    image_save_path = "F:\LYDINC\Data\Robot\image"
    video_path = "F:\\LYDINC\\AI\\robot_lydinc\\Data processing\\origin"
    image_from_video = "F:\\LYDINC\\AI\\robot_lydinc\\Data processing\\captured"
