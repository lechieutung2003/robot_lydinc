...
def evaluation_model(model, X_test, y_test, message):
    ...
    with open(".\\evaluate\\modelLogisticRegression.txt", "a") as f:
        f.write(f"============= {message} =============")
        f.write("\n")
...
# Initialize the LogisticRegression model with the best parameters
...
# Save the best model
joblib.dump(model, f".\\model\\databaseV5\\modelLogisticRegression_best.joblib")
